...
class Compiler:
    ...
    def __init__(self, bundle_directory: str | os.PathLike[str] | None = None):
        ...
        self._driver = ctypes.CDLL(str(self._driver_path))
        self._driver.main.argtypes = [ctypes.c_int, ctypes.POINTER(ctypes.c_char_p)]
        self._driver.main.restype = ctypes.c_int
        self._configure_api()
    ...
    def execute_tcc(self, arguments: list[str]) -> int:
        """Run the bundled TCC driver's exported main through ctypes."""
        encoded = [
        ...
        for index, argument in enumerate(encoded):
            argv[index] = argument
        return int(self._driver.main(len(encoded), argv))
