...
TARGETS = {
    ...
    "windows-x86_64": ("x86_64-win32-", ["-DTCC_TARGET_X86_64", "-DTCC_TARGET_PE"]),
    "windows-aarch64": ("arm64-win32-", ["-DTCC_TARGET_ARM64", "-DTCC_TARGET_PE"]),
    "macos-x86_64": ("x86_64-osx-", ["-DTCC_TARGET_X86_64", "-DTCC_TARGET_MACHO"]),
    "macos-aarch64": ("arm64-osx-", ["-DTCC_TARGET_ARM64", "-DTCC_TARGET_MACHO"]),
}
...
def runtime_sources(target: str, source_root: Path) -> list[Path]:
    ...
    if pe:
        files.extend([
            "chkstk.S", "crt1.c", "crt1w.c", "wincrt1.c", "wincrt1w.c",
            "dllcrt1.c", "dllmain.c", "winex.c",
        ])
    files.extend(["stdatomic.c", "atomic.S", "builtin.c", "alloca.S", "alloca-bt.S"])
    base = source_root / "win32" / "lib" if pe else source_root / "lib"
    root_runtime_sources = {
        "libtcc1.c", "lib-arm64.c", "stdatomic.c", "atomic.S", "builtin.c",
        "alloca.S", "alloca-bt.S", "dsohandle.c", "va_list.c", "armflush.c",
    }
    resolved = []
    for filename in files:
        path = base / filename
        if not path.is_file() and filename in root_runtime_sources:
...
def main() -> int:
    ...
    for target, (prefix, defines) in TARGETS.items():
        ...
        driver_arguments = [
            str(options.compiler.resolve()), "-B", str(native_runtime),
            "-I", str(source_root / "include"), "-I", str(source_root),
            "-shared", "-rdynamic", "-DTCC_DRIVER_DLL", "-DONE_SOURCE=1",
            *defines, f'-DCONFIG_TCC_CROSSPREFIX="{prefix}"',
        ]
        if not windows_host:
